...
export class StatusBadgeComponent {
  ...
  get label(): string {
    if (this.type === 'order' && this.orderStatus)
      return ORDER_STATUS_LABELS[this.orderStatus];
  ...
  get bgColor(): string {
    ...
    const map: Record<string, string> = {
      paid: '#10B98120', unpaid: '#F59E0B20',
      refunded: '#6B728020', failed: '#EF444420',
    };
  ...
  get textColor(): string {
    if (this.type === 'order' && this.orderStatus)
      return ORDER_STATUS_COLORS[this.orderStatus];
    const map: Record<string, string> = {
      paid: '#10B981', unpaid: '#F59E0B',
      refunded: '#6B7280', failed: '#EF4444',
    };
